...
PPPMonthScaledPounds2023 <- ggplot(STMonthSoftShell2023, aes(x = Month)) + 
  ...
  scale_y_continuous(name = "Mean Price per Pound \n(2023 FPI-adjusted)",
    sec.axis = sec_axis(~ . * scaling_factor, name = "Total Pounds Harvested", labels = comma)) +
...
# Prepare the data with seasonal and temporal variables
seasonal_props <- new_data %>%
  mutate(
    date = mdy(`Legal Notice Date`),  # Parse mm/dd/yyyy format
    year = year(date),
    month = month(date),
    ...
    time_period = case_when(
      year <= 2016 ~ "Early (2014-2016)",
      year <= 2019 ~ "Middle (2017-2019)",
      year <= 2022 ~ "Late (2020-2022)",
      TRUE ~ "Recent (2023-2025)"),
    time_period = factor(time_period, levels = c("Early (2014-2016)", "Middle (2017-2019)", 
                                                 "Late (2020-2022)", "Recent (2023-2025)")))
```
...
p2 <- seasonal_props %>%
  ...
  scale_fill_brewer(type = "qual", palette = "Set1", name = "Season") +
  scale_x_continuous(breaks = seq(min(seasonal_props$year[seasonal_props$year != 2025]), 
                                  max(seasonal_props$year[seasonal_props$year != 2025]), 
                                  by = 1)) +
  labs(title = "Changes in Seasonal Timing of Closures Over Time",
